...
const quickEventPopup = document.querySelector(".popup_quickEvent");
const closeBtnQuickEvent = document.getElementById("closeBtnQuickEvent");
...
function closePopup() {
    ...
    quickEventPopup.style.display = "none";
...
function displayCalendar() {
    ...
    for (let i = 1; i <= sumAllDays; i++ ) {
        ...
        if (i < (firstDayIndex + 1)) {
            dayCell.innerHTML = `<p>${weekdaysArr[indexWeekdays]}, ${prevLastDay - countPrevDays + 1}</p>`;
            dayString = `${prevLastDay - countPrevDays + 1}/${date.getMonth()-1}/${date.getFullYear()}`;
            countPrevDays--;
            indexWeekdays++;
        } else if (i <= lastDay && indexWeekdays < 7) {
            dayCell.innerHTML = `<p>${weekdaysArr[indexWeekdays]}, ${i - firstDayIndex}</p>`;
            dayString = `${i - firstDayIndex}/${date.getMonth()}/${date.getFullYear()}`;
            indexWeekdays++;
        } else if (i <= lastDay + firstDayIndex) {
            dayCell.innerHTML = `<p>${i - firstDayIndex}</p>`;
            dayString = `${i - firstDayIndex}/${date.getMonth()}/${date.getFullYear()}`;
        } else { 
            dayCell.innerHTML = `<p>${i - firstDayIndex - lastDay}</p>`;
        ...
        if (eventForDay) {
            dayCell.style.backgroundColor = "#C2E4FE";
            ...
            dayCell.appendChild(eventP);
            dayCell.appendChild(nameP);
        } 

        daysCalendar.appendChild(dayCell);
        ...
        dayCell.addEventListener("mouseleave", function(event) {
            if (eventP.textContent || nameP.textContent) {
                event.target.style.backgroundColor = "#C2E4FE";
            }
        });
        dayCell.addEventListener('click', () => openPopup(dayString));
    }
}
...
addBtn.addEventListener("click", function() {
    quickEventPopup.style.display = "block";
});
